...
class geoEmbedding(nn.Module):
    '''
    geo embedding
    geo_idx: [num_vertex, geo_seq_length]
    D:      output dims
    ...
    def __init__(self, vocab_size, D):
        ...
        self.emb_SE = nn.Embedding(num_embeddings=vocab_size, embedding_dim=D)
        # self.lstm = nn.LSTM(input_size=D, hidden_size=D, batch_first=True)

    def forward(self, geo_idx):
        # spatial embedding
        SE = self.emb_SE(geo_idx)  # [N, geo_sq_len, D]
        # SE, (h_n, c_n) = self.lstm(SE)
        # pooling [N, D]
...
class STEmbedding(nn.Module):
    ...
    def __init__(self, D, bn_decay=0.1):
        ...
        #  dealing the sequence of char embedding
        self.se = nn.Linear(768, D)
        self.FC_se = FC(
            input_dims=[D, D], units=[D, D], activations=[F.relu, None],
            bn_decay=bn_decay)

        self.transformer = nn.Transformer(d_model=D, nhead=8, num_encoder_layers=6)
    ...
    def forward(self, SE, TE):
        # spatial embedding; N L D
        SE = self.se(SE)
        SE = self.transformer(SE, SE)  # N L D'
        SE = torch.mean(SE, dim=1) # N D'
        spa_emb = SE.unsqueeze(0).unsqueeze(0)
        # print("SE.shape = ", SE.shape)
        spa_emb = self.FC_se(spa_emb)
        TE = self.FC_te(TE)
        
        return torch.add(spa_emb, TE), SE
...
class USPGCN_MultiD_Inductive_Tattr(nn.Module):
    ...
    def __init__(self, input_length, number_of_feature, filters, device, output_length, sg_num, cg_num):
        ...
        # self.geo_emb = geoEmbedding(32, 16)
    ...
    def forward(self, X, TE, position, weather, A_s, A_t, is_training=True):
        # X:(B, T, N, C') embedding X
        X = self.fc_x(X)
        B, T, N, D = X.shape
        # embedding position and TE
        # SE = self.geo_emb(position)
        STE, SE = self.ste_emb(position, TE)
        # SE = SE.squeeze()
        ...
        X_weather, _ = self.cross_attention(query=X, key=weather, value=weather)
        ...
        return output, rep, attn_list, zc_list, SE

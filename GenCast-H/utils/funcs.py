...
class MyDatasetTatt_CL(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if x.shape[0] == 0:
            print(f"[WARNING] Empty x at index {index}")
        if x1.shape[0] == 0:
            print(f"[WARNING] Empty x1 at index {index}")
        if y.shape[0] == 0:
            print(f"[WARNING] Empty y at index {index}")
        if te.shape[0] == 0:
            print(f"[WARNING] Empty te at index {index}")
        if weather.shape[0] == 0:
            print(f"[WARNING] Empty weather at index {index}")
...
def hashing_geolocation(lat, lon, hash_size=6):
    geohash_vec = np.vectorize(geohash2.encode)
    geohashes = geohash_vec(lat, lon, precision=hash_size)
    # print("geohashes", geohashes)
    return geohashes

def char2idx(geohashes, base="0123456789bcdefghjkmnpqrstuvwxyz"):

    char_idx = {ch: i for i, ch in enumerate(base)}
    indices = []
    for geohash in geohashes:
        idx = [char_idx[ch] for ch in geohash]
        indices.append(idx)
    return indices
...
def load_mel_data(feat_fn, sensor_fn, adj_type="connectivity", normalized_k=0.05):
    ...
    geo_hashes = hashing_geolocation(lat, lon)
    geo_idxs = char2idx(geo_hashes, base="0123456789bcdefghjkmnpqrstuvwxyz")
    ...
    return feat, sensors,dist_mx,adj_mx, geo_idxs
...
def load_pems_data_np(feat_fn,sensor_fn, normalized_k = 0.05):
    ...
    return df, sensors,dist_mx,adj_mx, geo_idxs

...
def compute_derivative_loss(output, p, t, fspd, compute_delta=False, delta=5):
    ...
    # print(args.quantile)
    if delta is not None and delta > 0 and args.quantile < 1:
        loss = torch.mean(F.huber_loss(loss_raw, torch.zeros_like(loss_raw), delta=delta, reduction='none'))
    else:
        loss = torch.mean(loss_raw ** 2)
        # print(f"Using MSE loss for physical loss computation.")
...
def train_predict(args, loader, model, optimizer, criterion, device, A_s, A_t, know_sensor_list, masked_sensor_ids,
                  his_length, weight, latlon, fspd_o, compute_delta=False):
    batch_loss = 0
    ...
    for idx, (inputs_m, inputs_s, targets, time_mat, weather) in enumerate(loader):
        ...
        latlon.requires_grad_(True)

        # (B,T,N,C)
        outputs_m, rep_m, attn_list, _ = model(inputs_m, time_mat, latlon, weather, A_s, A_t)
        _, rep_s, _, _ = model(inputs_s, time_mat, latlon, weather, A_s, A_t)
        ...
        sim_matrix = torch.exp(sim_matrix / 0.5)
        ...
        phys_loss, phy_raw = compute_derivative_loss(outputs_m, latlon, time_mat, fspd_o, compute_delta, args.delta)
    ...
    if compute_delta:
        ...
        if args.delta == 0:
            logging.warning("Delta is zero, using MSE loss instead of Huber loss.")
...
@torch.no_grad()
def test_predict(loader, model, device, A_s, A_t, know_list, unknow_list, his_length, latlon):
    test_pred = []
    test_gt = []
    ...
    for idx, (inputs_m, inputs_s, targets, time_mat, weather) in enumerate(loader):
        ...
        outputs, _, _, _ = model(inputs_m, time_mat, latlon, weather, A_s, A_t)
...
if __name__ == "__main__":
    ...
    logging_fn = '{}/{}/GenCastL_{}_cg_{}_lamda_{}_ar{}_{}_s{}_stype{}_lr{}_lw{}_unknown_ratio{}_theta{}_lb{}_B{}_quantile{}'.format(args.log_dir,args.dataset,str(args.spatial_group),str(args.channel_group),str(args.lamda),str(args.aug_ratio),str(args.a_sg_nk),str(args.seed),str(args.split_type),str(args.lr),str(args.lweight),str(args.unknown_ratio),str(args.theta),str(args.look_back), str(args.batch_size), str(args.quantile))
    saving_dir = '{}/{}'.format(args.model_dir,args.dataset)
    model_saving_fn = '{}/{}/GenCastL_{}_cg_{}_lamda_{}_ar{}_{}_s{}_stype{}_lr{}_lw{}_unknown_ratio{}_theta{}_lb{}_B{}_quantile{}.pkl'.format(args.model_dir,args.dataset,str(args.spatial_group),str(args.channel_group),str(args.lamda),str(args.aug_ratio),str(args.a_sg_nk),str(args.seed),str(args.split_type),str(args.lr),str(args.lweight),str(args.unknown_ratio),str(args.theta),str(args.look_back), str(args.batch_size), str(args.quantile))
    ...
    if args.dataset == "metr_la":
        ...
        f_dir = dat_dir
        region_path = dat_dir + 'prompt_embeddings_road_wo_address_limited_poi.npy'
        ...
        feat, sensor_ids, sensor_dist, adj_s,latlon = load_metr(feat_fn, sensor_ids_fn, normalized_k=0.05)
        _, _, _, adj_s_sg,_ = load_metr(feat_fn, sensor_ids_fn, args.a_sg_nk)
        spa_emb = np.load(region_path)
    ...
    latlon = torch.from_numpy(latlon.astype(np.float32)).to(device)

    # weather = weather[:, :total_day * Td]
    # (35136, 12, 400, 4)
    feat_valid = feat[know_valid, :]
    weather_valid = weather[:, :, know_valid, :]
    latlon_valid = latlon[know_valid, :]
    ...
    logging.info("Initial weight = {}".format(lweight))
    ...
    logging.info("Finish training! Final weight = {}".format(lweight))
